	...
	void CheckCollision() {
		...
		if (topHit != null) {
			if (topHit.gameObject.tag == MyTags.PLAYER_TAG) {
				if (!stunned) {
					topHit.gameObject.GetComponent<Rigidbody2D> ().linearVelocity =
						new Vector2 (topHit.gameObject.GetComponent<Rigidbody2D>().linearVelocity.x, 7f);
		...
		if (leftHit) {
			if (leftHit.collider.gameObject.tag == MyTags.PLAYER_TAG) {
				if (!stunned) {
					...
					if (tag != MyTags.BEETLE_TAG) {
						myBody.linearVelocity = new Vector2 (15f, myBody.linearVelocity.y);
